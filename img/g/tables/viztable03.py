...
alls = mats.keys()#["holdnack","alderton","chabris (a)","chabris (b)","maccann","ibrahim","baumgartner","marsh"]
...
plt.yticks([0,50,100])
#plt.legend()
plt.legend(frameon=False,loc="lower right", title="cognitive tests")
#
...
plt.legend(frameon=False,loc="lower right", title="physical tests")
plt.yticks([0,50,100])
plt.xticks([0,50,100])
#
...
for matlab in alls:
    ...
    if matlab in pers:
        ...
        plt.plot(100*np.arange(1,len(e)+1)/len(e), 100*np.cumsum(e)/np.sum(e), line, linewidth=0.8, markersize=2, label=matlab, alpha = 0.8)
plt.xlabel("% principal components")
#plt.setp(axis3.get_xticklabels(), visible=False)
#plt.ylim([0,1])
#plt.title("personality tests")
plt.legend(frameon=False,loc="lower right", title="personality tests")
plt.yticks([0,50,100])
plt.xticks([0,50,100])
plt.setp(axis1.get_xticklabels(), visible=False)
#from IPython import embed; embed()
#plt.setp(axis1.get_xlabel(), visible=False)
plt.setp(axis2.get_xticklabels(), visible=False)
#plt.title("general factors of intelligence or fitness?")
