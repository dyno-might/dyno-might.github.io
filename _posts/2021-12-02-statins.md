...
The easiest place to find effect sizes should be small-molecule drugs: There's no interaction with lifestyle, you can control for placebo effects with placebo pills, and these experiments are what the medical system, in its grace, liked to spend money on.
...
*Aside*: Statins have an interesting mechanism of action: [HMG-Coa reductase](https://en.wikipedia.org/wiki/HMG-CoA_reductase) is an enzyme that takes [HMC-Coa](https://en.wikipedia.org/wiki/HMG-CoA) and creates [mevalonic acid](https://en.wikipedia.org/wiki/Mevalonic_acid), which is needed to make cholesterol. Statins are molecules that are structurally similar to HMC-Coa, meaning they sort of get stuck on the HMC-Coa reductase enzyme where HMC-Coa is supposed to go, meaning the enzyme can't make mevalonic acid, which bottlenecks the whole cholesterol pipeline.
...
Kristensen et al. do a meta-analysis of 11 studies. Let's take a quick look at a couple of them to get a feel for data. (We'll need the intuition later.)
...
(By the way, Kristensen et al. are well aware that their analysis might be conservative, and make the same points I do!)
