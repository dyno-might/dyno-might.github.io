...
tags: 
description: include them only when they mean something
...
First, try to accept that all axis lines are optional. I promise you, your readers will recognize a plot even without lines around it.

So say you're wondering if you should draw a line for the x-axis. Consider these plots:
...
You use the same principle for deciding if you should draw a y-axis. Mentally draw these lines and then ask yourself if people should be comparing the lengths:

![](/img/axes/xarrows.svg)

## Example: Years vs. GDP
...
Here, the correlation is an [R value](https://en.wikipedia.org/wiki/Coefficient_of_determination)—0 means a test tells you nothing about life outcomes, and a 1 means the test allows you to predict life outcomes perfectly. So this suggests the Big Four is similar to Myers-Briggs scores, but maybe a little more informative.
