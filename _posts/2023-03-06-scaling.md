...
excerpt: It's hard not to feel blinkered by recent AI progress. Every week there seems to be an AMAZING NEW SYSTEM with UNPRECEDENTED CAPABILITIES. It's impossible not to wonder what the future holds. Until recently, I thought progress was so dizzying and unpredictable that the best bet was to throw all the details out the window and just rely on a simple Outside View. Something like, "The longer AI systems keep getting better, the longer we should expect them to continue getting better".But after looking into things, I was wrong. We know enough to form a credible Inside View"
permalink: /scaling/
...
*Update* (July 2023): Wondering how GPT-4 would fare on this benchmark? Unfortunately, it *can't* be evaluated in a meaningful way. All the BigBench documents include a long random "canary" string ([you can find it here](https://github.com/google/BIG-bench/blob/main/docs/paper/BIG-bench.tex#L588)). If you ask GPT-4, it can tell you that string. That means that OpenAI probably *included the benchmark in their training data*, so any evaluations on it would be meaningless.
