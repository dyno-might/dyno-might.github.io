...
* And what about `E`? Well, "slices" in Python exclude the endpoint, so `1:4` is equivalent to `[1,2,3]` which is equivalent to `i`, and so `E` is the same as `B`. Hahaha, just kidding! `E` has shape `10×3×2×1×40`.
...
def multi_head_attention(X, W_q, W_k, W_v, W_o):  
    ...
    final = projected.transpose(1, 0, 2).reshape(  
        seq_len, input_dim)  
    return final, output, projected
```
