...
comment:
  ...
  fortranlang: "https://fortran-lang.discourse.group/t/complaints-about-numpy/9735/4"
  lemmy: "https://old.lemmy.world/post/29703557"
head: '<style>
.language-python .n {
    color: #4A4A4A;
}
...
![np.linalg.solve](/img/numpy/solve.png)
...
Why did NumPy end up with a `np.linalg.solve(A,B)` function that's so confusing? I imagine they first made it work when `A` is a 2D array and `B` is a 1D or 2D array, just like the mathematical notation of `A⁻¹b` or `A⁻¹B`.
...
Also, it's probably wise to keep some distance between one's raving polemics and one's constructive array language API proposals. So I'll cover my new thing [next time](/dumpy).