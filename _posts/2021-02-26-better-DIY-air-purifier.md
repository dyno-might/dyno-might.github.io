...
* **Big fan**: Keep the same amount of filter, but run a bigger/faster motor that to push air through it faster.
* **Big filter**: Keep the same fan, but install more filters in parallel. This will decrease the pressure gradient at each of them, meaning more total airflow.

If we want to be quiet and energy-efficient, the choice is clear: Use a relatively weak fan, but with a filter large surface area.
