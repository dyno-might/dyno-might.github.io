...
Always, always, *always*. Thinking bad.
...
*P.S*. DumPy works out of the box with both [`jax.jit`](https://docs.jax.dev/en/latest/_autosummary/jax.jit.html) and [`jax.grad`](https://docs.jax.dev/en/latest/_autosummary/jax.grad.html). For gradients, you need to either cast the output to a JAX scalar or use the `dp.grad` wrapper.