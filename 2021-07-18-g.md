...
Let's do that. The facts are pretty simple. People who are good at one physical task also tend to be good at others. Faster runners tend to be able to do more push-ups. For mental tasks, the same positive correlations exist: People who are good at arithmetic also tend to have good memories.

Still, do these correlations have some deeper meaning? One theory is that you can model people's performance on all mental tasks using just a single number plus random noise. This is the idea behind the method called factor analysis, which is often described in very technical ways. However, as I'll explain, it all really boils down to the question of how much the data looks like a cigar.
...
To start, let's forget about intelligence, and ask an easier question: Does it make sense to refer to some people as "more physically fit" than others? I found three studies that gave people a bunch of different physical tests and make enough information available to analyze.
...
Are strong people also fast? Here's the correlations among males in Baumgartner and Zuidema's study. (Females are similar except with lower correlations in upper-body strength.)
...
Almost everything is positively correlated. Is this surprising? Perhaps not. But it gives a useful point of comparison.
...
Alderton et al. use a test battery designed to measure aptitude for various military tasks. Some of these, like tracking and target identification, are partly physical.

[![alderton 1997 correlations](/img/g/tables/alderton1997.svg)](/img/g/tables/alderton1997.pdf)

Here are the other three studies (click to open/close):
...
On the lower planes of argument, the main issue is if the tests are *just correlated* or if there's something deeper happening. One way to approach this is to ask how many numbers you need to describe someone. Suppose you've tested people on four physical tasks:
...
You can see that this data is special, in that the points fall along a straight line. This means you can represent each person using a single number, namely their position on the line.

### Simulations
{:.no_toc}

Now, say you wanted to simulate some fake data. A simple way would be to find some fixed direction of variation, and then randomly generate random points along it. Formally, let **w** be direction of variation, as a vector. To generate a person (i.e., a set of test scores), draw a random number **g** from a standard [Normal distribution](https://en.wikipedia.org/wiki/Normal_distribution) and then multiply it with **w**.

Since we have three tests, you need three numbers to specify a direction. If you use the direction **w = (1, -1, 1.5)**, you'll get a dataset that looks like this:
...
For contrast, here's how things would look if all the tests were totally uncorrelated.

[![independent model lengths](/img/g/tables/independent model_svds.svg)](/img/g/tables/independent model_svds.pdf)

What do the lengths on real data look like? Well, judge for yourself. Here are the results for Alderton et al.:
...
Calculating *g* is pretty similar to taking a simple average of the test scores, though the weights are *slightly* higher on some tasks than others.

If we calculate *g* like this for each person, we an then compute the *partial* correlations. These are the correlations once you control for *g*. Here's what that gives, again for Alderton et al.:
...
Factor analysis is a descent but not perfect model of mental tests. What does this tell us about how intelligence works? Well, suppose that factor analysis was a *perfect* model. Would that mean that we're all born with some single number *g* that determines how good we are at thinking?

No. A perfect fit would only mean that, across a population, a single number would *describe* how people do on tests (except from the "noise"). It does not mean that there's a single number or switch that *causes* test performance to be correlated.
